...
[logger.info](http://logger.info)("Notion 選項載入完成")
...
def is_url(t):     return bool([re.search](http://re.search)(r"https?://", t, re.I))

def is_youtube(t): return bool([re.search](http://re.search)(r"[youtube.com/watch|youtu.be/](http://youtube.com/watch|youtu.be/)", t, re.I))

def extract_yt_id(url):

m = [re.search](http://re.search)(r"[youtu.be/([A-Za-z0-9_-]+](http://youtu.be/([A-Za-z0-9_-]+))", url)

if m: return [m.group](http://m.group)(1)

m = [re.search](http://re.search)(r"[youtube.com/watch?.*v=([A-Za-z0-9_-]+](http://youtube.com/watch?.*v=([A-Za-z0-9_-]+))", url)
...
"🛠 貼文適用工具n已選：" + sel_text + "nn選完點「" + BTN_DONE + "」：",
...
"🎯 貼文重點n已選：" + sel_text + "nn選完點「" + BTN_DONE + "」：",
...
await update.message.reply_text(msg + "nn接下來選擇分類：")
...
await update.message.reply_text("⚠️ 主題不能為空，請點「" + BTN_EDIT_TITLE + "」輸入。")
...
[logger.info](http://logger.info)("Bot v13 starting...")

[app.run](http://app.run)_polling(drop_pending_updates=True)
